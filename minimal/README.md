# @indiepub/theme-minimal

A bare-bones [IndiePub](https://indiepub.dev) theme with no pre-built pages or components — bring your own design.

## What's included

- `package.json` with `@indiepub/astro` and `@indiepub/admin` dependencies
- `tsconfig.json` configured for Astro + TypeScript strict mode
- `src/env.d.ts` with Astro client types

Everything else is up to you. Add your own pages to `src/pages/`, your own components to `src/components/`, and your own styles. The `@indiepub/astro` integration handles content, feeds, and Micropub — you just build the UI.
...
Select **Minimal** when prompted for a template.
...
Add at least one page to `src/pages/` (e.g. `src/pages/index.astro`), then:
